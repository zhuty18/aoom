...
有的字体对于某个字符的排版效果处理不当，例如在思源宋体中，间隔号·的宽度大约为一个空格的宽度，而非应有的一个字的宽度（但方正书宋中则不会出现这个问题），可以用类似的方法强制规定宽度，建议使用字宽em为宽度单位。

```latex
\newcommand{\mysymbol}{\makebox[宽度]{符号}}
```
