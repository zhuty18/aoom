...
`book`类默认支持三种字号10,11,12pt，pt即为磅数。可以用“磅数÷2.845=毫米数”来计算文字大小。也可以参考word里面的五号字是10.5磅来选择。
...
对于一些在文档内使用生效的命令，如`\pagestyle`，需要在子文件的文档内容里使用，才能实现相同的渲染效果。在子文档中，页码自动从1开始。可以手动设置，但不建议这样做，因为手动调整页码的命令在主文件内同样生效，容易产生页码不连续的问题。
